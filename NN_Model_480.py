...
def createNeuralNetworkModel(input_size, output_size, learningRate):
    ...
    network = fully_connected(network, 128, activation="tanh", regularizer='L2',weights_init=tf.contrib.layers.variance_scaling_initializer(dtype=tf.float32),bias_init=tf.constant_initializer(value=0.5))
    #network = dropout(network, 0.8)

    network = fully_connected(network, 64, activation="tanh", regularizer='L2',weights_init=tf.contrib.layers.variance_scaling_initializer(dtype=tf.float32),bias_init=tf.constant_initializer(value=0.3))
    #network = dropout(network, 0.8)
